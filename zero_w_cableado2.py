...
# ── GPIO ───────────────────────────────────────────────────────────────────
def gpio_setup(pi: pigpio.pi):
    GPIO.setmode(GPIO.BCM)
    GPIO.setwarnings(False)
    for pin in RELAY_PINS:
        GPIO.setup(pin, GPIO.OUT, initial=GPIO.HIGH if ACTIVE_LOW else GPIO.LOW)
    # Pull-up interno (~50kΩ) en cada pin DATA del DHT22
    # Reemplaza la resistencia externa de 10kΩ mientras no la tengas.
    # Con resistencia externa podés sacar estas líneas.
    for pin in DHT_PINS:
        pi.set_pull_up_down(pin, pigpio.PUD_UP)
    print(f"[GPIO] Pull-up interno activado en pines {DHT_PINS}")
...
# ── Main ───────────────────────────────────────────────────────────────────
def main():
    # Inicializar pigpiod primero (necesario para pull-ups)
    pi = pigpio.pi()
    if not pi.connected:
